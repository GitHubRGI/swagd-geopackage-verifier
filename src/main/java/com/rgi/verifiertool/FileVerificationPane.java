...
public class FileVerificationPane extends TitledPane
{
    ...
    private VBox parent;
    private final Map<String, Collection<VerificationIssue>> fileErrorMessages = new HashMap<>();
     ...
     * Constructor
     *
     * @param geoPackageFile File handle to a GeoPackage file
     */
    public FileVerificationPane(final File geoPackageFile)
    {
        ...
        this.setOnMousePressed(this::createDeleteListener);
        //set the subsystem error messages/passing
        final VBox content = new VBox(7);
        this.setContent(content);
        content.setStyle(String.format("-fx-background-color: %s;", Style.greyBlue.getHex()));
        ...
        content.getChildren().addAll(subsystems);
        ...
        final Task<ToggleButton> mainTask = new Task<ToggleButton>()
        {
            ...
            protected ToggleButton call() throws Exception
            {
                try (final GeoPackage geoPackage = new GeoPackage(geoPackageFile, VerificationLevel.None, OpenMode.Open))
                {
                    ...
                    updateThreads.forEach(Thread::start);

                    updateThreads.forEach((ThrowingConsumer<Thread>) (Thread::join));
    ...
    private void createTitleAndButton()
    {
        ...
        title.prefWidthProperty().bind(new DoubleBinding()
        {
            ...
            protected double computeValue()
            {
                ...
                return VerifierMainWindow.getRootWidth() - breathingSpace;
            }
        });
    }
    ...
    private String getVerificationIssues()
    {
        ...
        final String body = this.fileErrorMessages.keySet().stream()
                .sorted(FileVerificationPane::compareSubsystems)
                .map(subsystem -> getVerificationIssuesForSubsystem(subsystem, this.fileErrorMessages.get(subsystem)))
        ...
        final String failedMessages = issues.stream()
                .sorted((issue1, issue2) -> issue1.getRequirement()
                        ...
                        .compareTo(issue2.getRequirement()
                                .reference()))
                .map(issue -> String.format("\t\r(%s) %s: \"%s\"\n\n\t\r%s\n",
                        issue.getSeverity(),
    ...
    private static Task<Collection<VerificationIssue>> createTask(final SubsystemVerificationPane subsystemVerificationPane, final GeoPackage geoPackage, final Map<String, Collection<VerificationIssue>> fileErrorMessages2)
    {
        ...
        task.valueProperty().addListener((observable, oldValue, newValue) -> subsystemVerificationPane.update(newValue));
